...
# from http://bernaerts.dyndns.org/linux/76-gnome/283-gnome-shell-install-extension-command-line-script
# --------------------------------------------
...
# the target folder
EXTENSION_FULL_PATH="${EXTENSION_PATH}/${EXTENSION_UUID}"

if [ "$EXTENSION_URL" != "" ]
then
    # don't override the folder if it already exists
    if [ ! -d "$EXTENSION_FULL_PATH" ]
    then
        # download extension archive
        wget --header='Accept-Encoding:none' -O /tmp/extension.zip "${GNOME_SITE}${EXTENSION_URL}"

        # unzip extension to installation folder
        mkdir -p $EXTENSION_FULL_PATH
        unzip /tmp/extension.zip -d "$EXTENSION_FULL_PATH"
    fi
